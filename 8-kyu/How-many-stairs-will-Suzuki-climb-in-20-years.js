...
function stairsIn20(s) {
  //   toal steps for each days in year
  const stepsForEachDayInYear = s.map((day) =>
    day.reduce((prev, current) => {
      return prev + current;
    })
  );
  // total steps for whole year
  const totalStepsForYear = stepsForEachDayInYear.reduce((prev, current) => {
    return prev + current;
  });
  // total steps for 20 yrs
  const totalStepsIn20yrs = totalStepsForYear * 20;
  return totalStepsIn20yrs;
}

// calling function by passing an argument stairs
